...
# @raycast.title Clipboard to Checklist
...
# Documentation:
# @raycast.description Extract tasks from clipboard text with Claude, create/update daily checklist in Bear
...
if ! command -v claude &> /dev/null; then
    ...
    osascript -e 'display notification "Claude Code CLI not found" with title "Clipboard to Checklist" sound name "Basso"'
    exit 1
fi
...
if [ -z "$TRANSCRIPT" ]; then
    ...
    osascript -e 'display notification "Clipboard is empty" with title "Clipboard to Checklist" sound name "Basso"'
    exit 1
fi
...
Analyze this text and extract ALL tasks — both pending and completed.
...
if [ -z "$RESPONSE" ]; then
    ...
    osascript -e 'display notification "Claude extraction failed" with title "Clipboard to Checklist" sound name "Basso"'
    exit 1
fi
...
if [ $PARSE_EXIT -ne 0 ]; then
    ...
    osascript -e 'display notification "Failed to parse tasks" with title "Clipboard to Checklist" sound name "Basso"'
    exit 1
fi
...
if [ -n "$EXISTING_TITLE" ]; then
    ...
    osascript -e "display notification \"Checklist updated ($TASK_COUNT tasks)\" with title \"Clipboard to Checklist\" sound name \"Glass\""
else
    # CREATE new note
    ...
    echo "  Tag: #$DAILY_TAG"
    echo "  Tasks: $TASK_COUNT"
    osascript -e "display notification \"Checklist created: $NOTE_TITLE\" with title \"Clipboard to Checklist\" sound name \"Glass\""
fi
