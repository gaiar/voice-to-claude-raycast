...
# Documentation:
# @raycast.description Clean clipboard text with Ollama, create Bear note
...
if ! curl -s "$OLLAMA_HOST" > /dev/null 2>&1; then
    ...
    osascript -e 'display notification "Ollama is not running. Please start it first." with title "Clipboard to Bear" sound name "Basso"'
    exit 1
fi
